...
def upgrade() -> None:
    ...
    sa.CheckConstraint("rarity IS NULL OR rarity IN (\n                'очень_редкая', 'редкая', 'необычная', 'обычная', 'распространенная'\n            )", name='check_race_rarity'),
    sa.CheckConstraint("size IN ('Крошечный', 'Маленький', 'Средний', 'Большой', 'Огромный', 'Гигантский')", name='check_race_size'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_race_abilities', 'races', ['racial_abilities'], unique=False, postgresql_using='gin')
    op.create_index('idx_race_languages', 'races', ['languages'], unique=False, postgresql_using='gin')
    op.create_index('idx_race_name_trgm', 'races', ['name'], unique=False, postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    op.create_index('idx_race_playable_size', 'races', ['is_playable', 'size'], unique=False)
    op.create_index('idx_race_regions', 'races', ['homeland_regions'], unique=False, postgresql_using='gin')
    op.create_index('idx_race_stat_bonuses', 'races', ['stat_bonuses'], unique=False, postgresql_using='gin')
    ...
    sa.CheckConstraint("social_rank IS NULL OR social_rank IN (\n                'император', 'король', 'князь', 'герцог', 'граф', 'боярин', \n                'рыцарь', 'дворянин', 'торговец', 'ремесленник', 'крестьянин', 'раб'\n            )", name='check_social_rank'),
    sa.CheckConstraint("status IN ('alive', 'dead', 'missing', 'legendary', 'unknown')", name='check_character_status'),
    sa.CheckConstraint("type IN ('npc', 'player', 'historical', 'deity', 'legendary', 'template')", name='check_character_type'),
    sa.CheckConstraint("wealth_level IS NULL OR wealth_level IN (\n                'нищий', 'бедный', 'средний', 'богатый', 'очень_богатый'\n            )", name='check_wealth_level'),
    sa.CheckConstraint('birth_year IS NULL OR birth_year > 0', name='check_birth_year'),
    sa.CheckConstraint('charisma IS NULL OR (charisma >= 1 AND charisma <= 30)', name='check_charisma_range'),
    sa.CheckConstraint('constitution IS NULL OR (constitution >= 1 AND constitution <= 30)', name='check_constitution_range'),
    sa.CheckConstraint('death_year IS NULL OR birth_year IS NULL OR death_year >= birth_year', name='check_death_after_birth'),
    sa.CheckConstraint('death_year IS NULL OR death_year > 0', name='check_death_year'),
    sa.CheckConstraint('dexterity IS NULL OR (dexterity >= 1 AND dexterity <= 30)', name='check_dexterity_range'),
    sa.CheckConstraint('intelligence IS NULL OR (intelligence >= 1 AND intelligence <= 30)', name='check_intelligence_range'),
    sa.CheckConstraint('level IS NULL OR (level >= 1 AND level <= 30)', name='check_level_range'),
    sa.CheckConstraint('strength IS NULL OR (strength >= 1 AND strength <= 30)', name='check_strength_range'),
    sa.CheckConstraint('wisdom IS NULL OR (wisdom >= 1 AND wisdom <= 30)', name='check_wisdom_range'),
    sa.ForeignKeyConstraint(['created_by_user_id'], ['users.id'], ),
    sa.ForeignKeyConstraint(['player_user_id'], ['users.id'], ),
    sa.ForeignKeyConstraint(['race_id'], ['races.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_character_created_by', 'characters', ['created_by_user_id', 'created_at'], unique=False)
    op.create_index('idx_character_faction_roles', 'characters', ['faction_roles'], unique=False, postgresql_using='gin')
    op.create_index('idx_character_name_trgm', 'characters', ['name'], unique=False, postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    op.create_index('idx_character_player_user', 'characters', ['player_user_id', 'type'], unique=False)
    op.create_index('idx_character_secondary_factions', 'characters', ['secondary_factions'], unique=False, postgresql_using='gin')
    op.create_index('idx_character_type_status', 'characters', ['type', 'status'], unique=False)
    op.create_index(op.f('ix_characters_birth_location_name'), 'characters', ['birth_location_name'], unique=False)
    ...
    op.create_index(op.f('ix_characters_created_by_user_id'), 'characters', ['created_by_user_id'], unique=False)
    op.create_index(op.f('ix_characters_current_location_name'), 'characters', ['current_location_name'], unique=False)
    ...
    op.create_index(op.f('ix_characters_player_user_id'), 'characters', ['player_user_id'], unique=False)
    op.create_index(op.f('ix_characters_primary_faction_name'), 'characters', ['primary_faction_name'], unique=False)
    ...
    sa.CheckConstraint("type IN (\n                'торговая_династия', 'военный_клан', 'религиозная_группа', 'дворянский_род',\n                'торговая_гильдия', 'ремесленная_гильдия', 'военный_орден', 'рыцарский_орден',\n                'княжеский_дом', 'королевская_династия', 'императорский_дом',\n                'племенной_союз', 'городское_правительство', 'республика',\n                'тайное_общество', 'культ', 'секта', 'братство',\n                'пиратская_флотилия', 'разбойничья_банда', 'наемная_компания'\n            )", name='check_faction_type'),
    sa.CheckConstraint("wealth_level IS NULL OR wealth_level IN (\n                'нищая', 'бедная', 'скромная', 'средняя', 'богатая', 'очень_богатая', 'баснословно_богатая'\n            )", name='check_wealth_level'),
    sa.CheckConstraint('decline_started_year IS NULL OR decline_started_year > 0', name='check_decline_year_positive'),
    sa.CheckConstraint('decline_started_year IS NULL OR founded_year IS NULL OR decline_started_year >= founded_year', name='check_decline_after_founding'),
    sa.CheckConstraint('fallen_year IS NULL OR fallen_year > 0', name='check_fallen_year_positive'),
    sa.CheckConstraint('fallen_year IS NULL OR founded_year IS NULL OR fallen_year >= founded_year', name='check_fall_after_founding'),
    sa.CheckConstraint('founded_year IS NULL OR founded_year > 0', name='check_founded_year_positive'),
    sa.ForeignKeyConstraint(['created_by_user_id'], ['users.id'], ),
    sa.ForeignKeyConstraint(['parent_faction_id'], ['factions.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_faction_achievements', 'factions', ['major_achievements'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_allies', 'factions', ['traditional_allies'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_cities', 'factions', ['controlled_cities'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_conflicts', 'factions', ['major_conflicts'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_current_allies', 'factions', ['current_allies'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_current_enemies', 'factions', ['current_enemies'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_description_fts', 'factions', ['description'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_diplomacy', 'factions', ['diplomatic_status'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_economic_assets', 'factions', ['economic_assets'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_enemies', 'factions', ['traditional_enemies'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_hierarchy', 'factions', ['parent_faction_id', 'type'], unique=False)
    op.create_index('idx_faction_historical', 'factions', ['founded_year', 'historical_importance'], unique=False)
    op.create_index('idx_faction_income_sources', 'factions', ['primary_income_sources'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_military_assets', 'factions', ['military_assets'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_name_trgm', 'factions', ['name'], unique=False, postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    op.create_index('idx_faction_power_analysis', 'factions', ['military_strength', 'economic_power', 'political_influence'], unique=False)
    op.create_index('idx_faction_regions', 'factions', ['controlled_regions'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_structure', 'factions', ['internal_structure'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_territorial', 'factions', ['primary_territory', 'status'], unique=False)
    op.create_index('idx_faction_trade_routes', 'factions', ['trade_routes_controlled'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_trade_spec', 'factions', ['trade_specialization'], unique=False, postgresql_using='gin')
    op.create_index('idx_faction_type_status', 'factions', ['type', 'status'], unique=False)
    op.create_index(op.f('ix_factions_created_by_user_id'), 'factions', ['created_by_user_id'], unique=False)
    op.create_index(op.f('ix_factions_current_leader_name'), 'factions', ['current_leader_name'], unique=False)
    ...
    op.create_index(op.f('ix_factions_historical_importance'), 'factions', ['historical_importance'], unique=False)
    op.create_index(op.f('ix_factions_leadership_type'), 'factions', ['leadership_type'], unique=False)
    op.create_index(op.f('ix_factions_military_strength'), 'factions', ['military_strength'], unique=False)
    op.create_index(op.f('ix_factions_name'), 'factions', ['name'], unique=False)
    op.create_index(op.f('ix_factions_parent_faction_id'), 'factions', ['parent_faction_id'], unique=False)
    op.create_index(op.f('ix_factions_political_influence'), 'factions', ['political_influence'], unique=False)
    op.create_index(op.f('ix_factions_primary_territory'), 'factions', ['primary_territory'], unique=False)
    ...
    sa.Column('population_composition', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    ...
    sa.CheckConstraint("wealth_level IS NULL OR wealth_level IN (\n                'нищая', 'бедная', 'скромная', 'средняя', 'богатая', 'очень_богатая', 'баснословно_богатая'\n            )", name='check_wealth_level'),
    sa.CheckConstraint('founded_year IS NULL OR founded_year > 0', name='check_founded_year_positive'),
    sa.CheckConstraint('garrison_size IS NULL OR garrison_size >= 0', name='check_garrison_positive'),
    ...
    op.create_index('idx_location_description_fts', 'locations', ['description'], unique=False, postgresql_using='gin')
    op.create_index('idx_location_events', 'locations', ['major_events'], unique=False, postgresql_using='gin')
    op.create_index('idx_location_landmarks', 'locations', ['cultural_landmarks'], unique=False, postgresql_using='gin')
    op.create_index('idx_location_languages', 'locations', ['languages_spoken'], unique=False, postgresql_using='gin')
    op.create_index('idx_location_name_trgm', 'locations', ['name'], unique=False, postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    op.create_index('idx_location_resources', 'locations', ['natural_resources'], unique=False, postgresql_using='gin')
    op.create_index('idx_location_routes', 'locations', ['trade_routes'], unique=False, postgresql_using='gin')
    op.create_index('idx_location_strategic_military', 'locations', ['strategic_importance', 'fortification_level'], unique=False)
    op.create_index('idx_location_trade_goods', 'locations', ['main_trade_goods'], unique=False, postgresql_using='gin')
    op.create_index('idx_location_type_region', 'locations', ['type', 'region'], unique=False)
    op.create_index('idx_location_wealth_population', 'locations', ['wealth_level', 'population'], unique=False)
    op.create_index(op.f('ix_locations_climate'), 'locations', ['climate'], unique=False)
    op.create_index(op.f('ix_locations_controlling_faction_name'), 'locations', ['controlling_faction_name'], unique=False)
    op.create_index(op.f('ix_locations_created_by_user_id'), 'locations', ['created_by_user_id'], unique=False)
    op.create_index(op.f('ix_locations_current_ruler_name'), 'locations', ['current_ruler_name'], unique=False)
    op.create_index(op.f('ix_locations_current_status'), 'locations', ['current_status'], unique=False)
    op.create_index(op.f('ix_locations_danger_level'), 'locations', ['danger_level'], unique=False)
    op.create_index(op.f('ix_locations_fortification_level'), 'locations', ['fortification_level'], unique=False)
    op.create_index(op.f('ix_locations_founded_year'), 'locations', ['founded_year'], unique=False)
    op.create_index(op.f('ix_locations_government_type'), 'locations', ['government_type'], unique=False)
    ...
    op.create_index(op.f('ix_locations_strategic_importance'), 'locations', ['strategic_importance'], unique=False)
    ...
    sa.CheckConstraint("article_type IN (\n                'персонаж', 'локация', 'фракция', 'раса', 'событие', 'легенда', \n                'история', 'культура', 'религия', 'политика', 'экономика',\n                'военное_дело', 'магия', 'технология', 'язык', 'обычаи',\n                'артефакт', 'организация', 'правила', 'хроника', 'биография',\n                'географический_справочник', 'исторический_документ', 'законы',\n                'торговые_пути', 'генеалогия', 'справочник'\n            )", name='check_article_type'),
    sa.CheckConstraint("canonical_status IN ('canon', 'semi_canon', 'non_canon', 'alternative', 'disputed')", name='check_canonical_status'),
    sa.CheckConstraint("category IS NULL OR category IN (\n                'персонажи', 'география', 'история', 'политика', 'культура', \n                'религия', 'экономика', 'военное_дело', 'магия', 'расы',\n                'фракции', 'технологии', 'языки', 'обычаи', 'артефакты',\n                'справочники', 'правила_игры'\n            )", name='check_article_category'),
    sa.CheckConstraint("difficulty_level IN ('beginner', 'easy', 'medium', 'hard', 'expert')", name='check_difficulty_level'),
    sa.CheckConstraint("reliability_level IS NULL OR reliability_level IN (\n                'verified', 'reliable', 'mostly_reliable', 'questionable', 'unreliable', 'fictional'\n            )", name='check_reliability_level'),
    sa.CheckConstraint("source_type IN (\n                'original', 'translated', 'adapted', 'compiled', 'referenced', 'player_created'\n            )", name='check_source_type'),
    sa.CheckConstraint("status IN ('draft', 'review', 'published', 'archived', 'deleted')", name='check_article_status'),
    sa.CheckConstraint("visibility_level IN ('public', 'players_only', 'gm_only', 'private')", name='check_visibility_level'),
    sa.CheckConstraint('reading_time_minutes IS NULL OR reading_time_minutes >= 0', name='check_reading_time_positive'),
    sa.CheckConstraint('time_period_end IS NULL OR time_period_end > 0', name='check_end_period_positive'),
    sa.CheckConstraint('time_period_end IS NULL OR time_period_start IS NULL OR time_period_end >= time_period_start', name='check_period_chronology'),
    sa.CheckConstraint('time_period_start IS NULL OR time_period_start > 0', name='check_start_period_positive'),
    ...
    sa.ForeignKeyConstraint(['primary_character_id'], ['characters.id'], ),
    sa.ForeignKeyConstraint(['primary_faction_id'], ['factions.id'], ),
    sa.ForeignKeyConstraint(['primary_location_id'], ['locations.id'], ),
    sa.ForeignKeyConstraint(['primary_race_id'], ['races.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_article_category_published', 'articles', ['category', 'is_published'], unique=False)
    op.create_index('idx_article_content_fts', 'articles', ['content'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_created_by', 'articles', ['created_by_user_id', 'created_at'], unique=False)
    op.create_index('idx_article_cultural_context', 'articles', ['cultural_context'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_languages', 'articles', ['languages_mentioned'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_last_modified_by', 'articles', ['last_modified_by_user_id', 'updated_at'], unique=False)
    op.create_index('idx_article_period', 'articles', ['historical_period', 'time_period_start', 'time_period_end'], unique=False)
    op.create_index('idx_article_publication', 'articles', ['is_published', 'publication_date'], unique=False)
    op.create_index('idx_article_related_chars', 'articles', ['related_characters'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_related_factions', 'articles', ['related_factions'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_related_locs', 'articles', ['related_locations'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_related_races', 'articles', ['related_races'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_religious_context', 'articles', ['religious_context'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_sources', 'articles', ['source_documents'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_summary_fts', 'articles', ['summary'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_tags', 'articles', ['tags'], unique=False, postgresql_using='gin')
    op.create_index('idx_article_title_trgm', 'articles', ['title'], unique=False, postgresql_using='gin', postgresql_ops={'title': 'gin_trgm_ops'})
    op.create_index('idx_article_type_status', 'articles', ['article_type', 'status'], unique=False)
    op.create_index('idx_article_visibility', 'articles', ['visibility_level', 'is_public'], unique=False)
    op.create_index(op.f('ix_articles_article_type'), 'articles', ['article_type'], unique=False)
    op.create_index(op.f('ix_articles_canonical_status'), 'articles', ['canonical_status'], unique=False)
    ...
    op.create_index(op.f('ix_articles_difficulty_level'), 'articles', ['difficulty_level'], unique=False)
    op.create_index(op.f('ix_articles_historical_period'), 'articles', ['historical_period'], unique=False)
    ...
    op.create_index(op.f('ix_articles_last_modified_by_user_id'), 'articles', ['last_modified_by_user_id'], unique=False)
    op.create_index(op.f('ix_articles_primary_character_id'), 'articles', ['primary_character_id'], unique=False)
    op.create_index(op.f('ix_articles_primary_faction_id'), 'articles', ['primary_faction_id'], unique=False)
    op.create_index(op.f('ix_articles_primary_location_id'), 'articles', ['primary_location_id'], unique=False)
    op.create_index(op.f('ix_articles_primary_race_id'), 'articles', ['primary_race_id'], unique=False)
    ...
    op.create_index(op.f('ix_articles_visibility_level'), 'articles', ['visibility_level'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index('idx_article_title_trgm', table_name='articles', postgresql_using='gin', postgresql_ops={'title': 'gin_trgm_ops'})
    ...
    op.drop_index('idx_location_name_trgm', table_name='locations', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    ...
    op.drop_index('idx_faction_name_trgm', table_name='factions', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    ...
    op.drop_index('idx_character_secondary_factions', table_name='characters', postgresql_using='gin')
    op.drop_index('idx_character_player_user', table_name='characters')
    op.drop_index('idx_character_name_trgm', table_name='characters', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    ...
    op.drop_index('idx_race_name_trgm', table_name='races', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
