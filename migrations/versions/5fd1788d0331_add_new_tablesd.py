...
def upgrade() -> None:
    ...
    sa.CheckConstraint("category IN ('racial', 'class', 'spell', 'feat', 'item', 'condition', 'custom')", name='check_ability_category'),
    sa.CheckConstraint('charisma_modifier IS NULL OR (charisma_modifier >= -10 AND charisma_modifier <= 10)', name='check_charisma_modifier_range'),
    sa.CheckConstraint('constitution_modifier IS NULL OR (constitution_modifier >= -10 AND constitution_modifier <= 10)', name='check_constitution_modifier_range'),
    sa.CheckConstraint('dexterity_modifier IS NULL OR (dexterity_modifier >= -10 AND dexterity_modifier <= 10)', name='check_dexterity_modifier_range'),
    sa.CheckConstraint('intelligence_modifier IS NULL OR (intelligence_modifier >= -10 AND intelligence_modifier <= 10)', name='check_intelligence_modifier_range'),
    sa.CheckConstraint('level_requirement IS NULL OR (level_requirement >= 1 AND level_requirement <= 20)', name='check_level_requirement_range'),
    sa.CheckConstraint('resource_cost IS NULL OR resource_cost >= 0', name='check_resource_cost_positive'),
    sa.CheckConstraint('save_dc IS NULL OR save_dc > 0', name='check_save_dc_positive'),
    sa.CheckConstraint('strength_modifier IS NULL OR (strength_modifier >= -10 AND strength_modifier <= 10)', name='check_strength_modifier_range'),
    sa.CheckConstraint('wisdom_modifier IS NULL OR (wisdom_modifier >= -10 AND wisdom_modifier <= 10)', name='check_wisdom_modifier_range'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_ability_category', 'abilities', ['category'], unique=False)
    op.create_index('idx_ability_class_requirement', 'abilities', ['class_requirement_id'], unique=False)
    op.create_index('idx_ability_description_fts', 'abilities', ['description'], unique=False, postgresql_using='gin')
    op.create_index('idx_ability_level_requirement', 'abilities', ['level_requirement'], unique=False)
    op.create_index('idx_ability_name_trgm', 'abilities', ['name'], unique=False, postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    op.create_index('idx_ability_race_requirement', 'abilities', ['race_requirement_id'], unique=False)
    ...
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.CheckConstraint("type IN ('боец', 'маг', 'жрец', 'разбойник', 'следопыт', 'варвар', 'бард', 'паладин', 'колдун', 'друид')", name='check_class_type'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_class_name_trgm', 'classes', ['name'], unique=False, postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    ...
    op.create_table('entity_abilities',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.CheckConstraint("entity_type IN ('character', 'race', 'class', 'faction', 'location', 'item')", name='check_entity_type'),
    sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index('idx_entity_ability_entity', 'entity_abilities', ['entity_type', 'entity_id'], unique=False)
    op.create_index('idx_entity_ability_type', 'entity_abilities', ['entity_type'], unique=False)
    op.create_index(op.f('ix_entity_abilities_ability_id'), 'entity_abilities', ['ability_id'], unique=False)
    op.create_index(op.f('ix_entity_abilities_entity_id'), 'entity_abilities', ['entity_id'], unique=False)
    op.create_index(op.f('ix_entity_abilities_entity_type'), 'entity_abilities', ['entity_type'], unique=False)
    ...
    op.create_foreign_key(None, 'character_game_stats', 'classes', ['subclass_id'], ['id'], ondelete='SET NULL')
    op.create_foreign_key(None, 'character_game_stats', 'classes', ['class_id'], ['id'], ondelete='SET NULL')
...
def downgrade() -> None:
    ...
    op.drop_index('idx_class_name_trgm', table_name='classes', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    ...
    op.drop_index('idx_ability_name_trgm', table_name='abilities', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
